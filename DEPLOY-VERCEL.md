...
- **缓存**：`/api/news` 的 5 分钟边缘缓存退化为函数实例内的内存缓存（Vercel Edge 未提供 `caches.default` 时自动垫片），功能不受影响。注意：原 `worker/index.js` 写缓存用的是 `response.clone()`，在 Edge 上「返回客户端 + 写缓存」并发读同一流会偶发把**空体**写进缓存、进而被 Vercel CDN 缓存成空 200；已改为先 `JSON.stringify` 缓冲成字符串、再据同一字符串独立构造「返回用」与「缓存用」两份 Response 以消除竞态，并在适配层加了「空响应自愈」（空体自动 `?fresh=1` 重算并回写好数据）。
